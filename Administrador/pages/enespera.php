                            ...
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                <tbody>
                                    <?php
                                        ...
                                        $query="SELECT V_Id, MDP_Id,MDP_Banco,MDP_Cantidad,MDPT_numerodetransferencia,MDPT_fechadetransferencia from venta,medio_de_pago where V_Id=MDP_Venta AND V_Estatus LIKE 'En espera' AND MDP_Confirmar=1 AND MDP_Tipo LIKE 'Transferencia'";
                        ...
                        <div class="panel-body">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                <thead>
                                    <tr>
                                        <th>Id venta</th>
                                        <th>Total de Venta(Bsf)</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php
                                        include 'conexion.php';
                                        $query="SELECT DISTINCT V_Id, V_Total from venta,medio_de_pago where V_Id=MDP_Venta AND V_Estatus LIKE 'En espera' AND MDP_Confirmar=1 AND MDP_Tipo LIKE 'Deposito'";
                                        $result = $mysqli -> query($query);
                                        $j=0;
                                        while($f = $result->fetch_array(MYSQLI_ASSOC)){
                                            $matriz[$j][0]=$f['V_Id'];
                                            $matriz[$j][1]=$f['V_Total'];
                                            $j++;
                                        }
                                        for ($i = 0; $i < $j; $i++) {
                                                echo "<tr class=\"odd gradeX\">";
